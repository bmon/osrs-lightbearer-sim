...
(() => {
  ...
  function transformer2() {
    var x0 = 0, x1 = 0.5, x2 = 1, s = 1, t0, t1, t2, k10, k21, interpolator = identity2, transform2, clamp = false, unknown;
    function scale(x) {
      return isNaN(x = +x) ? unknown : (x = 0.5 + ((x = +transform2(x)) - t1) * (s * x < s * t1 ? k10 : k21), interpolator(clamp ? Math.max(0, Math.min(1, x)) : x));
    }
    scale.domain = function(_) {
      return arguments.length ? ([x0, x1, x2] = _, t0 = transform2(x0 = +x0), t1 = transform2(x1 = +x1), t2 = transform2(x2 = +x2), k10 = t0 === t1 ? 0 : 0.5 / (t1 - t0), k21 = t1 === t2 ? 0 : 0.5 / (t2 - t1), s = t1 < t0 ? -1 : 1, scale) : [x0, x1, x2];
    };
    scale.clamp = function(_) {
    ...
    return function(t) {
      transform2 = t, t0 = t(x0), t1 = t(x1), t2 = t(x2), k10 = t0 === t1 ? 0 : 0.5 / (t1 - t0), k21 = t1 === t2 ? 0 : 0.5 / (t2 - t1), s = t1 < t0 ? -1 : 1;
      return scale;
    };
  }
  function diverging() {
  ...
  // src/sim.ts
  var NORMAL_THRESHOLD = 50;
  var LB_THRESHOLD = 25;
  var MAX_DURATION = 150;
  function simulateLb(equipPos, duration) {
    let timer2, threshold;
  ...
  function update(minTick) {
    ...
    if (bTick !== null) {
      ...
      breakevenLabel.attr("x", bx + 4).text(`Breakeven: ${bTick} ticks (${sec}s)`).attr("display", null);
    } else {
